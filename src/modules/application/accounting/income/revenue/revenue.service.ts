...
export class RevenueService {
  ...
  async findAllCustomerRevenue(
    ...
    try {
      ...
      return {
        success: true,
        message: 'Customer Wise Revenues fetched successfully',
        data: formattedData,
        meta: {
          total,
          page,
  ...
  async update(
    ...
    try {
      ...
      const updatedRevenue = await this.prisma.$transaction(async (tx) => {
        ...
        const newBankId =
          dto.bank_account_id !== undefined ? dto.bank_account_id : oldBankId;
        const newCustomerId =
          dto.customer_id !== undefined ? dto.customer_id : oldCustomerId;
        ...
        const updated = await tx.revenue.update({
          ...
          data: {
            ...
            ...(dto.invoice_category_id !== undefined && {
              invoice_category_id: dto.invoice_category_id,
            }),
        ...
        if (oldBankId && oldAmount)
          await tx.bankAccount.update({
            where: { id: oldBankId },
            data: { opening_balance: { decrement: oldAmount } },
          });
        if (newBankId && newAmount) {
          await tx.bankAccount.update({
            where: { id: newBankId },
            data: { opening_balance: { increment: newAmount } },
          });
          ...
          if (bank?.chart_of_account_id) {
            await tx.chartOfAccount.update({
              ...
              data: {
                balance: {
                  increment:
                    newAmount - (oldBankId === newBankId ? oldAmount : 0),
                },
              },
            });
          }
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        if (revenue.bank_account_id && revenue.amount) {
          await tx.bankAccount.update({
            ...
            data: { opening_balance: { decrement: revenue.amount } },
          });
          const bank = await tx.bankAccount.findUnique({
            where: { id: revenue.bank_account_id },
          });
