...
export class CurrencySettingsController {
  ...
  async create(
    @Request() req,
    @Body() dto: CreateCurrencySettingDto,
    @Res() res: Response,
  ): Promise<Response<CurrencySettings>> {
    ...
    if (type !== 'OWNER')
      return res
        ...
        .json({
          success: false,
          message: 'Only owners can create currency settings.',
        });
    ...
    try {
      const result = await this.currencySettingsService.create(
        dto,
        owner_id,
        workspace_id,
      );
  ...
  async get(
    @Request() req,
    @Res() res: Response,
  ): Promise<Response<CurrencySettings | null>> {
    const { id: owner_id, workspace_id, type } = req.user;
    if (type !== 'OWNER')
      return res
        .status(400)
        .json({
          success: false,
          message: 'Only owners can view currency settings.',
        });
    if (!owner_id || !workspace_id)
      return res
        .status(400)
    ...
    try {
      const settings =
        await this.currencySettingsService.getByOwnerAndWorkspace(
          owner_id,
          workspace_id,
        );
      if (!settings)
        return res
          .status(404)
          .json({ success: false, message: 'Currency settings not found.' });
  ...
  async update(
    ...
    if (type !== 'OWNER')
      return res
        ...
        .json({
          success: false,
          message: 'Only owners can update currency settings.',
        });
    if (!owner_id || !workspace_id)
      return res
        .status(400)
    ...
    try {
      const result = await this.currencySettingsService.update(
        id,
        dto,
        owner_id,
        workspace_id,
