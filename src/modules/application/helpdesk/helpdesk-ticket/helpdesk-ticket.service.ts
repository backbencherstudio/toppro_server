...
export class HelpDeskTicketService {
  ...
  async addDescriptionWithAttachments(
    ticketId: string,
    description: string,
    ...
    // ✅ 2. Only admin (SUPERADMIN) or ticket's owner/customer can add description
    if (
      userType !== 'SUPERADMIN' &&
      ticket.createdBy !== userId &&
      ticket.customerId !== userId
    ) {
      throw new ForbiddenException('You are not allowed to add description');
    }

    // ✅ 3. Create description
    const newDescription =
      await this.prisma.ticketDescriptionWithAttachment.create({
        data: {
          description,
          ticket: { connect: { id: ticketId } }, // strictly link to ticket
        },
      });

    // ✅ 4. Upload attachments
    if (files && files.length > 0) {
      await this.prisma.descriptionAttachment.createMany({
        data: files.map((file) => ({
          file_name: file.originalname,
          file_url: `uploads/${file.filename}`, // adjust to your storage logic
          file_size: file.size,
    ...
    // ✅ 5. Return description with its attachments
    return this.prisma.ticketDescriptionWithAttachment.findUnique({
      where: { id: newDescription.id },
      include: { attachments: true },
    });
  }

  // ➤ Get ticket with descriptions and attachments
  async getTicketWithDescriptions(ticketId: string, userId: string, userType: string) {
    const ticket = await this.prisma.helpDeskTicket.findUnique({
      where: { id: ticketId },
      include: {
        category: true,
    ...
    // OWNER can only see their own workspace ticket
    if (userType === 'OWNER' && ticket.customerId !== userId && ticket.createdBy !== userId) {
